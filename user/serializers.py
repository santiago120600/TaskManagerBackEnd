...
class TaskSerializer(serializers.ModelSerializer):
    user_name = serializers.CharField(source='user.name_user')
    folder_name = serializers.CharField(source='folder.name_folder')
    subtasks = SubTaskSerializer('subtasks',many=True)
    class Meta:
        model = Task
        fields =  ('id_task','img_task','desc_task','completed','user','user_name','folder','folder_name','created_at','updated_at', 'subtasks')

class UserSerializer(serializers.ModelSerializer):
    tasks = TaskSerializer(source='users',many=True)
    class Meta:
        model = User
        fields = '__all__'
